...
import { NoCreditsWrapper } from '@/components/interview/no-credits-wrapper';
...
export default async function NewInterviewPage({
  ...
  // Redirect only users who have never purchased (no credits, no history)
  // Users who have purchased but ran out will see the recap modal
  if (!subscription.hasPurchased && !subscription.canStartInterview) {
    redirect('/settings?tab=billing&reason=limit_reached');
  }

  // Fetch advanced features data for purchasers (resume targeting, job descriptions)
  ...
  // All purchasers get access to advanced features
  ...
  return (
    <NoCreditsWrapper
      hasCredits={subscription.canStartInterview}
    ...
    >
      <div className="w-full">
        ...
        <InterviewSetupForm
          ...
          hasPurchased={subscription.hasPurchased}
          voiceModeEnabled={subscription.hasPurchased}
