...
- **Supabase auth cookies are not `HttpOnly`** — `@supabase/ssr`'s `DEFAULT_COOKIE_OPTIONS` sets `httpOnly: false` (verified in `node_modules/@supabase/ssr/dist/main/utils/constants.js`), and none of `lib/supabase/server.ts`, `lib/supabase/middleware.ts`, or `lib/client.ts` override it. Flagged during the 2026-07-13 audit-checklist walk (§7); the missing `Secure` flag in the same finding was fixed the same session, but `httpOnly` deliberately was not. Reason: `lib/client.ts`'s browser client performs sign-in/sign-up/sign-out/token-refresh via `document.cookie`, which cannot set `HttpOnly` — forcing it server-side alone would just get silently clobbered by the browser client's next write. A real fix means moving all client-driven auth actions into server-side Route Handlers so only the server ever touches the cookie — a genuine architecture change, and also the officially documented `@supabase/ssr` + Next.js App Router pattern (not a misconfiguration unique to this repo). Note the JWT itself remains readable via `supabase.auth.getSession()` client-side regardless of this flag (the SDK needs it in JS for `Authorization` headers), so `httpOnly` would only close the narrow raw-`document.cookie`-read vector — the CSP (§10) is the app's actual primary XSS defense. Needs a product/architecture decision before implementing. **Chris wants this tackled together with the timezone item above, in one future session.**
- **Rate limiting is fully implemented but inactive in production** — `lib/rate-limit.ts`'s Upstash sliding-window limiter has correct per-route policies for every endpoint the checklist calls out (tts, stt, codeRun, codeSubmit, resumeUpload, chat, coaching, analyze, jdParse, webhookTest), but `RATE_LIMIT_ENABLED` / `UPSTASH_REDIS_URL` / `UPSTASH_REDIS_TOKEN` are confirmed NOT set in the live Vercel production environment (confirmed directly with Chris during the 2026-07-13 audit-checklist walk, §9 — cannot be checked from the sandbox). This means every rate-limited endpoint currently has zero abuse protection live in production: Judge0 code execution (pay-per-submission), TTS, Mistral resume/coaching/analysis calls, and DeepSeek JD parsing can all be looped without limit, and the webhook-test endpoint (`/api/webhooks/[webhookId]/test`) can be used as a DDoS reflector against an arbitrary third-party URL. No code change needed — this is purely a missing external service (Upstash Redis) + three Vercel env vars. Chris is setting this up himself; this line should be removed once he confirms it's live.
